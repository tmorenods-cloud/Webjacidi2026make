...
import { Outlet, Link, ScrollRestoration, useNavigate } from "react-router";
import { SectionContact } from "./global/SectionContact";
import { Footer } from "./global/Footer";

// ─── Logo corporativo (wordmark) ──────────────────────────────────────────────
function LogoCorporativo({ height = 44 }: { height?: number }) {
...
function StaticHeader({ visible }: { visible: boolean }) {
  ...
  return (
    <div
      className="header-estatico bg-white relative shrink-0 w-full hidden md:block"
      style={{
        opacity: visible ? 1 : 0,
      ...
      <div className="contenedor-header-estatico flex flex-row items-center size-full">
        <div className="contenido-header-estatico flex items-center justify-between pb-8 pt-7 px-20 w-full">
          ...
          <div className="nav-header-estatico flex gap-6 items-center">
            ...
            <span onClick={() => scrollTo("servicios")} className="nav-link-servicios font-medium text-[#20201f] text-[18px] tracking-[-0.54px] cursor-pointer">Servicios</span>
            ...
            <button onClick={() => scrollTo("contacto")} className="boton-contacto-header bg-[#20201f] flex items-center justify-center px-4 py-[10px] rounded-[29px]">
  ...
  return (
    ...
    >
      ...
      <div className="nav-sticky-container flex gap-5 items-center">
        ...
        <button onClick={() => scrollTo("contacto")} className="boton-sticky-contacto bg-[#20201f] flex items-center justify-center px-4 py-2 rounded-[29px]">
...
function MobileHeader() {
  ...
  const glassStyle: React.CSSProperties = {
    ...
    backgroundColor: "rgba(255,255,255,0.85)",
    border: "1px solid rgba(32,32,31,0.15)",
  };
    ...
    <div className="header-mobile fixed top-4 left-0 right-0 z-50 flex flex-col items-center px-4 md:hidden">
      ...
      >
        ...
        <Link to="/" onClick={() => setOpen(false)} className="logo-mobile shrink-0">
          <IsotipoCorporativo size={32} />
        </Link>

        {/* Burger — dos líneas minimalistas */}
        <button
          className="burger-mobile flex flex-col justify-center items-end gap-[6px] w-8 h-8 shrink-0"
          onClick={() => setOpen((v) => !v)}
          aria-label="Menú"
        >
          <span
            className="burger-line block bg-[#20201f] rounded-full"
            style={{
              height: "1.5px",
            ...
            >
              {to ? (
                <Link
                  ...
                  className="nav-link-mobile block font-medium text-[#20201f] py-3 border-b border-[rgba(32,32,31,0.08)] last:border-0"
                ...
                <span
                  className="nav-link-mobile block font-medium text-[#20201f] py-3 border-b border-[rgba(32,32,31,0.08)] cursor-pointer"
                  style={{ fontSize: "18px", letterSpacing: "-0.03em" }}
